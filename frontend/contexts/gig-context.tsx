...
export interface Gig {
  ...
  status: "pending" | "accepted" | "in-progress" | "delivered" | "completed" | "disputed"
...
interface GigContextType {
  gigs: Gig[]
  createGig: (
    gigData: Omit<Gig, "id" | "client" | "freelancer" | "status" | "escrowFunded" | "postedDate">,
  ) => Promise<void>
  ...
  resolveDispute: (gigId: number, winner: "client" | "freelancer") => Promise<void>
  getGigsByClient: (clientAddress: string) => Gig[]
  getGigsByFreelancer: (freelancerAddress: string) => Gig[]
  getAvailableGigs: () => Gig[]
  getDisputedGigs: () => Gig[]
  getPlatformStats: () => { totalGigs: number; totalVolume: number; totalFees: number; activeDisputes: number }
  isLoading: boolean
}
...
const initialGigs: Gig[] = [
  {
    ...
    description: "Need a modern landing page for my startup with responsive design and smooth animations",
    price: "0.5",
    currency: "ETH",
    deadline: "2024-02-15",
  ...
  },
  {
    ...
    description: "Develop and deploy an ERC-20 token with custom tokenomics and security features",
    price: "1.2",
    currency: "ETH",
    deadline: "2024-02-25",
    category: "Blockchain",
  ...
  },
  {
    ...
    description: "Design a complete mobile app interface for a fitness tracking application",
    price: "0.8",
    currency: "ETH",
    deadline: "2024-02-18",
    category: "Design",
  ...
  },
  {
    ...
    description: "Build a custom WordPress site for a local business with e-commerce functionality",
    price: "1.5",
    currency: "ETH",
    deadline: "2024-02-08",
    category: "Web Development",
...
export function GigProvider({ children }: { children: React.ReactNode }) {
  ...
  const fundEscrow = async (gigId: number) => {
    ...
    try {
      ...
      setGigs((prev) =>
        prev.map((gig) => (gig.id === gigId ? { ...gig, escrowFunded: true, status: "in-progress" as const } : gig)),
      )
    } finally {
      setIsLoading(false)
    }
  ...
  const resolveDispute = async (gigId: number, winner: "client" | "freelancer") => {
    setIsLoading(true)
    try {
      // Simulate smart contract interaction
      await new Promise((resolve) => setTimeout(resolve, 1500))
  ...
  const getGigsByClient = (clientAddress: string) => {
    return gigs.filter((gig) => gig.client.toLowerCase() === clientAddress.toLowerCase())
  }

  const getGigsByFreelancer = (freelancerAddress: string) => {
    return gigs.filter((gig) => gig.freelancer?.toLowerCase() === freelancerAddress.toLowerCase())
  }
  ...
  const getPlatformStats = () => {
    ...
    const activeDisputes = gigs.filter((gig) => gig.status === "disputed").length
